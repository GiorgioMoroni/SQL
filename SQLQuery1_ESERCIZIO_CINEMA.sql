...
/*
	...
	la durata e la classificazione per età. Questa vista aiuterà il personale e i clienti a
...
/*
	...
	Questa informazione è cruciale per la direzione per valutare il successo commerciale dei film.
*/
...
/*
	...
	Questo permetterà al personale e alla direzione di monitorare il feedback dei clienti in tempo reale.
*/
...
/*
	...
	La procedura dovrebbe verificare la disponibilità del posto e registrare l'acquisto.
*/
	...
	BEGIN TRY
		BEGIN TRANSACTION
			IF EXISTS (SELECT 1 FROM Ticket WHERE @spettacoloID = ShowtimeID AND @numeroPosto = SeatNumber)
				...
				THROW 50001, 'Posto già occupato', 1
...
/*
	...
	Questo include la possibilità di aggiungere o rimuovere spettacoli dall'agenda.
*/
	...
	BEGIN TRY
		BEGIN TRANSACTION
			IF EXISTS (SELECT 1 FROM Showtime WHERE @spettacoloID = ShowtimeID AND @salaID = TheaterID AND @orario = ShowDateTime )
				...
				THROW 50002, 'Spettacolo già esistente', 1
	...
	BEGIN TRY
		BEGIN TRANSACTION
			IF EXISTS (SELECT 1 FROM Movie WHERE @filmID = MovieID)
				...
				THROW 50003, 'Film già inserito', 1
			ELSE
